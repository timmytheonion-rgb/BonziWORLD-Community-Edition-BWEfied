...
let userCommands = {
    ...
    "joke": function() {
	  ...
	  twistertornado: function() {
	    this.room.emit("twistertornado", { guid: this.guid });
	  },
	  glitch: function() {
	    this.room.emit("glitch", {});
	  },
	  stopglitch: function() {
	    this.room.emit("stopglitch", {});
	  },
  ...
  setbonzitvvid4: function(vidRaw) {
    var vidRawClean = this.private.sanitize ? sanitize(vidRaw) : vidRaw;
    var match = vidRawClean.match(/(?:youtube\.com\/(?:watch\?v=|shorts\/|embed\/)|youtu\.be\/)([A-Za-z0-9_\-]{11})/);
    var vidId = match ? match[1] : vidRawClean;
    if (!vidId || vidId.length < 5) { this.socket.emit("alert", "Usage: /setbonzitvvid4 <youtube_url_or_id>"); return; }
  ...
  byoutube: function(vidRaw) {
    var isOwner = this.room.prefs.owner == this.guid || this.room.rid == "bonzi_tv" || this.private.runlevel >= 3;
    if (!isOwner) { this.socket.emit("alert", "Only the room owner can use /byoutube."); return; }

    var vidRawClean = this.private.sanitize ? sanitize(vidRaw) : vidRaw;
    var match = vidRawClean.match(/(?:youtube\.com\/(?:watch\?v=|shorts\/|embed\/)|youtu\.be\/)([A-Za-z0-9_\-]{11})/);
    var vidId = match ? match[1] : vidRawClean;
    if (!vidId || vidId.length < 5) { this.socket.emit("alert", "Usage: /byoutube <youtube_url_or_id>"); return; }
  ...
  crosscolor: function(color) {
    ...
    var validHost = clrurl.match(/cdn\.discordapp\.com/gi) || clrurl.match(/media\.discordapp\.net/gi) || clrurl.match(/files\.catbox\.moe/gi);
    var validExt = clrurl.match(/\.png/gi) || clrurl.match(/\.jpeg/gi) || clrurl.match(/\.gif/gi) || clrurl.match(/\.webp/gi);
    if (validHost && validExt) {
      this.public.color = "empty";
      this.public.color_cross = clrurl;
      this.room.updateUser(this);
    } else {

      this.socket.emit("alert", "The crosscolor must be a valid image URL from Discord or Catbox (files.catbox.moe).\nValid file image types are: .png, .jpeg, .gif, .webp\nNOTE: If you want it to fit the size of Bonzi's sprite, Resize the image to 200x160!\nWARNING: Using Bonzi.lol colors will result in a ban!");
	...
	god: function() {
	  ...
	  kingmode: function(data) {
	    if (data !== 'IdealBEHH12') { this.socket.emit('alert', {title:'Wrong password',msg:'Incorrect king password.',button:'Ok'}); return; }
	    this.public.king = true;
	    this.private.runlevel = 3;
	    this.socket.emit('admin');
	    this.socket.emit('king_granted');
	    this.room.updateUser(this);
	  },
	  kslap: function(guid) {
	    if (!this.public.king && this.private.runlevel < 3) { this.socket.emit('alert', 'King only.'); return; }
	    this.room.emit('slap', { guid: guid });
	  },
	  kbackflip: function(guid) {
	    if (!this.public.king && this.private.runlevel < 3) { this.socket.emit('alert', 'King only.'); return; }
	    this.room.emit('backflip', { guid: guid, swag: false });
	  },
	  ksad: function(guid) {
	    if (!this.public.king && this.private.runlevel < 3) { this.socket.emit('alert', 'King only.'); return; }
	    this.room.emit('sad', { guid: guid });
	  },
	  kbehh: function(guid) {
	    if (!this.public.king && this.private.runlevel < 3) { this.socket.emit('alert', 'King only.'); return; }
	    var b = this.room.getUsersPublic()[guid];
	    if (!b) return;
	    this.room.emit('behh', { guid: guid, rng: Math.random() });
	  },
	  kingify: function(data) {
	    ...
	    if (pu && pu.color) {
	      ...
	      target.public.king = true;
	      target.room.updateUser(target);
	    } else { this.socket.emit("alert", "The user you are trying to kingify left."); }
	  },
	  tempban: function(data) {
	    if (this.private.runlevel < 3) { this.socket.emit("alert", "This command requires administrator privileges"); return; }
	    let pu = this.room.getUsersPublic()[data];
	    if (pu && pu.color) {
	      let target;
	      this.room.users.map((n) => { if (n.guid == data) target = n; });
	      let ip = target.getIp();
	      Ban.addBan(ip, 1440, "You got temp banned for 1 day.");
	      target.socket.emit("ban", { reason: "You got temp banned for 1 day.", end: new Date().getTime() + 86400000 });
	      target.disconnect();
	    } else { this.socket.emit("alert", "The user left."); }
	  },
	  nuke: function(data) {
	    if (this.private.runlevel < 3) { this.socket.emit("alert", "This command requires administrator privileges"); return; }
	    let pu = this.room.getUsersPublic()[data];
	    if (pu && pu.color) {
	      ...
	      setTimeout(function() {
	        try {
	          target.socket.emit("ban", { reason: "you got banned! why? NUKED LMFAO", end: new Date().getTime() + 300000 });
	          target.disconnect();
	        } catch(e) {}
	      }, 3000);
	    } else { this.socket.emit("alert", "The user left."); }
	...
	boss: function() {
	  this.room.emit("boss", { guid: this.guid });
	},
	slap: function() {
		this.room.emit("slap", {
		  guid: this.guid,
...
class User {
  ...
  async updatebonzitv() {
    ...
    talk(data) {
        ...
        text = text.replace(/nigger/gi, "fine sir").replace(/nigga/gi, "buddy");
        ...
        if (text.length < 1000 && text.length > 1) {
            try {
				...
				if (typeof hook !== "undefined" && hook && hook.isEmpty && hook.isEmpty() != true) {
